...
int report_distance_info(struct s_sxz_distance *data)
{
    char *snd_buf;
    struct s_com *msg;
    int n;
    ...
    memset(snd_buf, 0, sizeof(char)*BUFLEN);
    msg = (struct s_com *)snd_buf;
    msg->code = XZ_DISTANCE;
    msg->id = 0; /* temporary set */
    msg->flags = 0;

    memcpy(snd_buf+sizeof(struct s_com), data, sizeof(struct s_sxz_distance));
...
int send_raw_data(void *data, int len)
{
    char *snd_buf;
    int n;
    
    snd_buf = (char *)malloc(sizeof(char)*len);
    if(snd_buf == NULL)
    {
        return -1;
    }
    memset(snd_buf, 0, sizeof(char)*len);
    memcpy(snd_buf, data, sizeof(char)*len);
    
    n = sendto(m_base.fd, snd_buf, len, 0, (struct sockaddr *) &m_base.m_addr, sizeof(struct sockaddr_in));
    free(snd_buf);
